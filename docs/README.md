...
- [x] 컨트롤러는 출력기, 입력기로부터 날짜, 메뉴 주문받음
- [x] 컨트롤러는 출력기에게 주문 받은 메뉴를 전달한다.
- [x] 출력기는 주문받은 메뉴와 개수를 출력한다.
- [x] 컨트롤러부터 주문받은 메뉴를 받아 주문 기계 객체를 생성한다.
- [x] 주문 기계는 할인 전 총금액을 계산한다.
- [x] 주문 기계는 출력기에게 할인 전 총 금액을 전달하고 출력한다.
...
- [x] 이벤트 플래너는 출력기에게 적용 이벤트들의 이름과 할인금액을 전달하고 출력기가 출력한다. (각 이벤트와 할인금액 출력)
- [x] 이벤트 플래너는 총혜택 금액을 계산하고 출력기에게 전달 후 출력한다.
- [x] 주문 기계는 할인 후 총금액을 계산한다.
- [x] 주문 기계는 출력기에게 할인 후 총금액을 전달한다.
- [x] 출력기는 할인 후 총금액을 출력한다.
- [x] 이벤트 플래너는 출력기에게 총혜택 금액에 따라 배지 이름을 전달한다.
- [x] 출력기는 총혜택 금액에 따라 적용된 배지 이름을 출력한다.
- [x] 끝

## 🗓️이벤트 플래너

- [x] 식당 방문 날짜, 메뉴를 선택 -> 이벤트 플래너는 `주문 메뉴`, `할인 전 총주문 금액`, `증정 메뉴`, `혜택 내역`, `총혜택 금액`, `할인 후 예상 결제 금액`, `12월 이벤트 배지` 내용
  출력
